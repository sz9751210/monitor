...
1. 取得憑證到期時間
2. 到期後發送通知
4. 透過 Telegram Bot 發送訊息
3. 讀取 yaml domain list
4. 設定環境變數
5. CloudFunction 設定
6. CloudScheduler 排程設定
...
`"%Y-%m-%d"` 是 `strftime` 方法的格式化字串參數，其中 `%Y` 表示 4 位數的年份，`%m`表示月份（01至12），`%d` 表示月份中的天數（01 至 31）。
...
🔗專案repo --> [ssl-certificate-checker](https://github.com/sz9751210/monitor/tree/main/monitor-ssl)
